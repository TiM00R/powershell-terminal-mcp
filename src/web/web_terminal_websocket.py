            ...
            while True:
                try:
                    ...
                    if message.get('type') == 'terminal_input':
                        # User typed in THIS terminal. Send raw to the local pwsh
                        # session; ConPTY echoes back to ALL terminals via broadcast.
                        input_data = message.get('data', '')
                        if input_data:
                            self.shared_state.send_manual_input(input_data)
                            logger.debug(f"Forwarded input: {repr(input_data[:20])}")

                    elif message.get('type') == 'terminal_resize':
                        # Terminal resized -> resize the ConPTY.
                        cols = message.get('cols')
                        rows = message.get('rows')
                        if cols and rows:
                            self.shared_state.resize(cols, rows)
